...
export default function SingleProduct() {
  ...
  useEffect(() => {
    fetch(process.env.REACT_APP_BACKEND_URL + `/product?id=${routeParams.id}`)
  ...
  // This is a functional component that returns a JSX element with the single product page
  return (
    <div>
      <Header />
      <main>
        <div id="singleProductPageContainer">
          <div id="imagePreview">
            <div
              class="imagePreviewBtn"
              onClick={() => setCurrentPreview(backendData[0].game_cover_url)}
            >
            ...
            {artworks.map((artwork) => (
              <div
                class="imagePreviewBtn"
                onClick={() => setCurrentPreview(artwork)}
              >
                <img src={artwork}></img>
              </div>
            ))}
