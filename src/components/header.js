...
export default function Header() {
  ...
  useEffect(() => {
    if (localStorage.getItem("currentUser") === null) {
      ...
      userNameInitial.current.textContent = localStorage
        .getItem("currentUser")
        .replaceAll('"', "")
        .toUpperCase()[0];
  ...
  // Navigation bar with a side navigation menu that appears on all pages except the homepage.
  return (
    <header id="header">
      <nav>
        <div id="sidenav" className="side-nav">
          ...
          <div className="side-nav-links">
            ...
            <a href="/login" ref={menuLoginBtn}>
              Login
            </a>
            <a href="/register" ref={menuRegisterBtn}>
              Register
            </a>
            <a href="/" onClick={logout} ref={menuLogoutBtn}>
              Logout
            </a>
          </div>
        </div>
